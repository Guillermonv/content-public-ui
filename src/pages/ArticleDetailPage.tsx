...
function SidebarArticle({ article, navigate }: { article: Article; navigate: (path: string) => void }) {
  ...
  return (
    <button
      ...
      className="group w-full flex gap-3 text-left rounded-xl p-2 -mx-2 hover:bg-gray-50 dark:hover:bg-gray-700/30 transition-colors"
    >
      <div className="w-20 h-16 flex-shrink-0 rounded-lg overflow-hidden bg-gray-100 dark:bg-gray-700">
        <img
          src={article.image}
          alt={article.title}
      ...
      <div className="flex-1 min-w-0 py-0.5">
        <span className={`inline-block text-[10px] font-bold uppercase tracking-wider px-1.5 py-0.5 rounded mb-1 ${meta.badgeBgClass}`}>
          {meta.label}
        </span>
    ...
    return (
      <div className="flex gap-10 items-start">
        <div className="flex-1 min-w-0 animate-pulse space-y-6 py-2">
          ...
          <div className="h-[380px] bg-gray-200 dark:bg-gray-700 rounded-2xl" />
          <div className="space-y-3">
            <div className="h-8 bg-gray-200 dark:bg-gray-700 rounded w-5/6" />
            <div className="h-8 bg-gray-200 dark:bg-gray-700 rounded w-4/6" />
          </div>
          ...
          <div className="space-y-3">
            {[...Array(10)].map((_, i) => (
              <div key={i} className="h-4 bg-gray-200 dark:bg-gray-700 rounded" style={{ width: `${98 - i * 2}%` }} />
            ))}
          </div>
        </div>
  ...
  return (
    <div className="flex gap-10 items-start">
      ...
      <div className="flex-1 min-w-0">
        ...
        {/* Hero image */}
        <div className="relative w-full aspect-[16/9] rounded-2xl overflow-hidden bg-gray-100 dark:bg-gray-800 mb-8 shadow-xl">
          <img
            src={article.image}
            alt={article.title}
          ...
          <div className="absolute top-4 left-4">
            <span className={`inline-flex items-center px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wide backdrop-blur-sm ${meta.badgeBgClass}`}>
              {meta.label}
            </span>
          </div>
          <div className="absolute bottom-4 right-4 flex items-center gap-1.5 bg-black/40 backdrop-blur-sm text-white text-xs font-medium px-3 py-1.5 rounded-full">
            <Clock size={11} />
        ...
        <div className="flex items-center justify-between gap-4 mb-8 pb-6 border-b border-gray-100 dark:border-gray-700">
          <div className="flex items-center gap-3 min-w-0">
            <img
              ...
              className="w-10 h-10 rounded-full object-cover ring-2 ring-primary-200 dark:ring-primary-800 flex-shrink-0"
            />
          ...
          <button
            ...
            className="flex items-center gap-1.5 px-4 py-2 rounded-full bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-300 text-sm font-medium hover:bg-primary-50 dark:hover:bg-primary-900/20 hover:text-primary-600 dark:hover:text-primary-400 transition-colors flex-shrink-0"
          >
            <Share2 size={14} />
            Compartir
          </button>
        ...
        <div className="flex items-center gap-2 flex-wrap mt-10 pt-8 border-t border-gray-100 dark:border-gray-700">
          ...
          <span className={`inline-flex items-center px-3 py-1 rounded-full text-xs font-semibold ${meta.badgeBgClass}`}>
            {meta.label}
          </span>
        </div>
